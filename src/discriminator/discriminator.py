...
def parse_arguments():
    ...
    parser.add_argument('--transformers_path', type=str, default='roberta-base/')
    parser.add_argument('--device', type=str, default='cuda:0')
    parser.add_argument('--n_splits', type=int, default=5)
    parser.add_argument('--test_size', type=float, default=0.3)
    parser.add_argument('--batch_size', type=int, default=64)
    parser.add_argument('--data_folder', type=str, default='results')
    parser.add_argument('--non_modified_data', type=str, default='experiment/generated.json')
    parser.add_argument('--non_modified_data_sample_size', type=int, default=1000)
...
def main(args):
    ...
    for path in paths:
        ...
        if bpt not in result.keys():
            result[bpt] = dict()
        result[bpt][method] = {'acc': list(), 'roc_auc': list(), 'f1score': list()}
        ...
        for j in range(args.n_splits):
            ...
            train_data, test_data = get_train_test_data(random.sample(non_modified_data, args.non_modified_data_sample_size), 
                                                        modified_data,
                                                        test_size=args.test_size, 
                                                        random_state=seed)
            transform = CustomTransform(tokenizer, max_len=100)
            train_dataset = CustomDataset(train_data[0], train_data[1], transform=transform)
            test_dataset = CustomDataset(test_data[0], test_data[1], transform=transform)
            ...
            config = AutoConfig.from_pretrained(args.transformers_path, num_labels = 2)
            model = AutoModelForSequenceClassification.from_pretrained(args.transformers_path, config = config).to(device)
