...
def vlc(args, model, tokenizer, start_word, private_code, m=10):
    ...
    input_prepared = model.prepare_inputs_for_generation(
    ...
    while len(private_code) > np.log2(m):
        ...
        sorted_, indices = torch.sort(
            soft(outputs.logits[:, -1, :]), descending=True)
        words_prob = [(j, i) for i, j in zip(
            sorted_[0][:m].tolist(), indices[0][:m].tolist())]
...
def main(args):
    ...
    for word_id, word in enumerate(most_common_first_words):
        ...
        if args.encoding_type == 'FLC':
            perp, generated = flc(
                args, model, tokenizer, word, private_code, args.m)
        elif args.encoding_type == 'VLC':
            perp, generated, n_of_codes = vlc(
                args, model, tokenizer, word, private_code, args.m)
    ...
    if args.encoding_type == 'VLC':
        metric['beats_per_token'] = len(
            most_common_first_words) * args.len_of_private_code / np.sum(number_of_codes_used)
    ...
    json.dump(
        ...
        Path(
            args.out_folder,
            'metrics',
            f"{args.encoding_type}_{args.m}.json").open('w'))
    json.dump(
        generated_strings,
        Path(
            args.out_folder,
